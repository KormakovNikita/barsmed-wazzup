...
- **WhatsApp Business** — напрямую через WhatsApp Web (Baileys), с SOCKS5/HTTP прокси для РФ
...
- **Статусы клиентов** — «Теплый», «Нецелевой», «Записался» и отдельный флаг **ВИП** (розовый диалог + красная метка)
- **Аналитика** — поступившие диалоги за период с разбивкой по мессенджерам (`/analytics`)
